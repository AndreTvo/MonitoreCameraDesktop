...
{
	if (point->X() >= Game::viewport.left && 
		point->X() < Game::viewport.right)
		if (point->Y() >= Game::viewport.top &&
			point->Y() < Game::viewport.bottom)
...
bool Renderer::Initialize(Window * window, Graphics * graphics) 
{
...
#ifdef _DEBUG
	hr = D3DReadFileToBlob(L"Vertex.cso", &vShader);
	...
	D3D11_INPUT_ELEMENT_DESC layoutDesc[] =
	{
		...
		{ "COLOR",    0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, offsetof(Vertex, pos), D3D11_INPUT_PER_VERTEX_DATA, 0 },
...
#ifdef _DEBUG
	hr = D3DReadFileToBlob(L"Pixel.cso", &pShader);
...
void Renderer::RenderBatch(ID3D11ShaderResourceView ** texture, SpriteInfo ** sprites, uint count)
{
	...
	graphics->Context()->PSSetShaderResources(0, 2, texture);
	...
	while (count > 0)
	{
		...
		// quantidade de imageSprite é maior do que o espaço disponível
		...
		{		
			...
			// os quatro vértices do imageSprite são calculados a partir de transformações dessas posições unitárias
			...
			for (int i = 0; i < VerticesPerSprite; ++i)
			{
				...
				// Write vector as a Float4, even though VertexPositionColor::vector is an XMFLOAT3.
