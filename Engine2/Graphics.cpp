...
bool Graphics::Initialize(Window * window)
{
	...
	swapDesc.BufferDesc.Width = uint(window->InternalWidth());	// largura do backbuffer
	swapDesc.BufferDesc.Height = uint(window->InternalHeight());// altura do backbuffer
	...
	depthStencilDesc.Width = uint(window->InternalWidth());		// largura da textura
	depthStencilDesc.Height = uint(window->InternalHeight());	// altura da textura 
	...
	viewport.TopLeftX = 0;
	viewport.TopLeftY = 0;
	viewport.Width = float(window->InternalWidth());
	viewport.Height = float(window->InternalHeight());
...
void Graphics::Resize(int width, int height)
{
	...
	hr = swapChain->GetBuffer(0, __uuidof(ID3D11Texture2D), reinterpret_cast<void**>(&pBuffer));
	// Perform error handling here!
