    ...
    int32_t connectionTimeoutInMilliseconds)
{
	...
	try
	{
		...
		for (;;)
		{
            ...
            {
				// We have no frame but we don't want to stop the stream
				// Wait for a proper frame instead of finishing stream
				if (!framePtr)
					continue;
            ...
            boost::this_thread::sleep_for(boost::chrono::milliseconds(streamPtr_->InterframeDelayInMilliseconds()));
		}
